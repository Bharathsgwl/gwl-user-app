...
class ExamSection extends React.Component {
   ...
   onClickLogoutAutomaticaly = () => {
    ...
    axios
      ...
      .then(response => {
        ...
        if (!response.data.isloggedIn) {
          history.push("/");
          console.log("history push is working");
        }
      })
  ...
  handleOnClickOkAns = () => {
    var { post, user,history } = this.props;
    debugger;
    axios
...
const mapStateToProps = ({ q_lists, post, user, exam_rules,history }) => {
  return {
    q_lists,
    post,
    exam_rules,
