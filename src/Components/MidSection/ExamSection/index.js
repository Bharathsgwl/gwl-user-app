...
state = {
  ...
  render() {
    const {question,index,value,disabled,hours,minutes,seconds,capture} = this.state
    const {handleOnEndTest,handleOnClickNext,handleChange,handleOnOptioncapture,componentWillMount,handleTimerStop,handleTimerStart,handleTimerCapture} = this;
    return (
      <React.Fragment >
      ...
      index = {index}
      disabled={disabled}
      handleChange = {handleChange} handleonNext={handleOnClickNext}/>
      </Grid>
      <Grid container >
      <Grid item md = {10} >
